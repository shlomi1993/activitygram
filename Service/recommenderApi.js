...
module.exports.predict_cf = async function(uid, k, userbased) {
	...
	await axios
		...
		.catch((error) => {
            console.error('predict_cf request failed:', error.response.status, error.response.statusText);
		});
...
module.exports.train_nn = async function(uid, train_file) {
	...
	await axios
		...
		.catch((error) => {
            console.error('train_nn request failed:', error.response.status, error.response.statusText);
		});
	return JSON.parse(result);
};
...
module.exports.predict_nn = async function(uid, test_file) {
	...
	await axios
		...
		.catch((error) => {
            console.error('predict_nn request failed:', error.response.status, error.response.statusText);
		});
	return JSON.parse(result);
};